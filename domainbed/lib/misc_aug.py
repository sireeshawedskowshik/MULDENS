...
def invenio_accuracy(algorithm,eval_dict, test_envs,correct_models_selected_for_each_domain,device, step, ensemble = False):
    correct = 0
    total = 0
    weights_offset = 0 
    ...
    save_dict = {'step': step, 'beta': beta,'models_selected': correct_models_selected_for_each_domain}
    pickle.dump(save_dict, open('test0_dict.pkl',"ab"))
    ...
    if ensemble:
        for i,test_env in enumerate(test_envs):
            for split in ['_in','_out']:
                ...
                loader= eval_dict[name][0]
                weights= eval_dict[name][1]
                acc=ensemble_accuracy(algorithm.invenio_networks,loader,weights,device)
                results[name+'_acc'] = acc
    else: 
        for i,test_env in enumerate(test_envs):
