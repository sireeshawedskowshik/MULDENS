...
for test_env in range(4):
    ...
    out_file = 'Inevnio_outputs_pacs_aug_M3/env'+str(test_env)+'/indiv_ensemble_no_beta/out copy.txt'
    out_df= pd.read_fwf(out_file)
    out_df= out_df[out_df['epoch']>0]
    train_envs = np.setdiff1d(np.arange(4),test_env)
    train_out_split_names= ['env'+str(i)+'_out' + str(j)+'_acc' for i in train_envs for j in range(total_train_val_domains)]
    ...
    print('test_train_Domain_val','test_oracle',out_df[test_ensemble_name].iloc[max_val_idx],out_df[test_ensemble_name].iloc[max_val_idx_1])